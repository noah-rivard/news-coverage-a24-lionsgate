...
- Multi-title content deals (international slates) now route to `content_deals.txt` and use the `format_content_deals` formatter, which preserves multiple titles and appends the article publish date when the model omits a date parenthetical (ignores unrelated parentheses such as subtitles).
