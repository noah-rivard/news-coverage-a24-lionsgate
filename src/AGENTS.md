...
- The coordinator pipeline (`workflow.process_article`) is single-article and stateless. It builds an OpenAI client only when default classifier/summarizer tools are used; injected tools plus/or a provided client allow offline tests. It routes prompts based on classifier output and writes ingest records to `data/ingest/{company}/{quarter}.jsonl` (409 duplicates surface to the user).
