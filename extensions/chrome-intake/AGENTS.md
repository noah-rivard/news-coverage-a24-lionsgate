...
- Service worker stores the latest scraped article in `chrome.storage.local` and sends it to the configured endpoint in `chrome.storage.sync` (default `http://localhost:8000/process/article`). It auto-sends after each scrape; the popup is mainly for status or retry.
...
- If the endpoint contains `/ingest/`, the service worker sends the coverage-schema payload including a required `facts` array (min 1). Otherwise it sends the raw article (title/source/url/content/published_at) for the `/process/article` pipeline.
