...
(() => {
  ...
  var linksMetaEl = document.getElementById("links_meta");
  ...
  function deriveServerBase(endpoint) {
  ...
  function openUrl(url) {
    chrome.tabs.create({ url }).catch(() => void 0);
  }
  function setDisabled(id, disabled) {
    const el = document.getElementById(id);
    if (el) el.disabled = disabled;
  }
  function bindOpen(id, urlProvider) {
    const el = document.getElementById(id);
    if (!el) return;
    el.onclick = () => {
  ...
  function renderLinks(endpoint) {
    const serverBase = deriveServerBase(endpoint);
    if (linksMetaEl) {
      linksMetaEl.textContent = serverBase ? `Endpoint: ${endpoint}
